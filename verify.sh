...
if [ $stage -le 0 ]; then
  ...
  python3 local/make_enroll.py entry
...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc.sh --write-utt2num-frames true --mfcc-config conf/mfcc.conf --nj $nj --cmd "$train_cmd" \
    entry/feature exp/make_mfcc $mfccdir
  utils/fix_data_dir.sh entry/feature
  sid/compute_vad_decision.sh --nj $nj --cmd "$train_cmd" \
    entry/feature exp/make_vad $vaddir
  utils/fix_data_dir.sh entry/feature
...
if [ $stage -le 3 ]; then
  $train_cmd exp/scores/log/scoring.log \
    ...
    "ark:ivector-subtract-global-mean $nnet_dir/xvectors_train/mean.vec scp:entry/xvector/xvector.scp ark:- | transform-vec $nnet_dir/xvectors_train/transform.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    ...
    " |" exp/scores_voxceleb1_test  || exit 1;
fi
